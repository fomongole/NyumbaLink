...
// ── Constants ─────────────────────────────────────────────────────────────────
const API_BASE = 'https://rentora-houselink-api-production.up.railway.app/api/v1';
const APP_URL  = 'https://rentora-houselink-admin.vercel.app';
...
export async function generateMetadata({
  ...
  if (!property) {
    return { title: 'Property Not Found — Rentora' };
  }
...
export default async function PublicPropertyPage({
  ...
  const shareText = encodeURIComponent(
    `${property.title} — UGX ${Number(property.price).toLocaleString()} / ${property.billingCycle ? BILLING_LABELS[property.billingCycle] ?? 'month' : 'month'}\n${property.area}, ${property.district.name}\n\nFound on Rentora: ${APP_URL}/p/${id}`,
  );
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
              ...
              {property.billingCycle && (
                <p className="text-sm text-gray-400 mt-0.5">
                  per {BILLING_LABELS[property.billingCycle] ?? property.billingCycle}
                </p>
              )}
              {isHostel && (
