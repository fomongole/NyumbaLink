...
import {
  ...
  CalendarClock, Calendar, Star, Building2,
  Wifi, Zap, Droplets, ShieldCheck, Dumbbell, Waves,
  UtensilsCrossed, Shirt, Tv, Wind, ArrowUpDown, Trees,
  Package, Globe, CheckCircle2, Flame, Coffee,
} from 'lucide-react';
import { API_BASE, APP_URL, PLAY_STORE_URL } from '@/lib/constants';
import SiteFooter from '@/components/shared/site-footer';
import SiteHeader from '@/components/shared/site-hearer';
import PropertyImageGallery from '@/components/public-property/property-image-gallery';
...
function getAmenityIcon(name: string): React.ElementType {
  ...
  if (/wifi|wi-fi|internet|broadband/.test(key))         return Wifi;
  if (/electric|power|electricity/.test(key))            return Zap;
  if (/generator|genset|backup power/.test(key))         return Zap;
  if (/water|borehole|tap/.test(key))                    return Droplets;
  if (/security|guard|cctv|surveillance/.test(key))      return ShieldCheck;
  if (/parking|garage|car/.test(key))                    return Car;
  if (/gym|fitness|workout/.test(key))                   return Dumbbell;
  if (/pool|swimming/.test(key))                         return Waves;
  if (/kitchen|cooking/.test(key))                       return UtensilsCrossed;
  if (/laundry|washing/.test(key))                       return Shirt;
  if (/tv|cable|dstv|television/.test(key))              return Tv;
  if (/air con|a\/c|ac|air-con|aircon/.test(key))        return Wind;
  if (/lift|elevator/.test(key))                         return ArrowUpDown;
  if (/garden|yard|lawn/.test(key))                      return Trees;
  if (/storage|store/.test(key))                         return Package;
  if (/hot water|geyser|heater/.test(key))               return Flame;
  if (/coffee|cafeteria|café/.test(key))                 return Coffee;
  if (/globe|fiber|fibre/.test(key))                     return Globe;
  return CheckCircle2; // sensible default — not Wifi
}
...
export default async function PublicPropertyPage({
  ...
  // Only show a clean address — omit if it's just a numeric ID
  const cleanAddress = property.address && !/^\d+$/.test(property.address.trim())
    ? property.address
    : undefined;
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        {/* ── Image Gallery (client component: slider + lightbox) ── */}
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-6 min-w-0">
            <div>
              ...
              <p className="mt-2 flex items-center gap-1.5 text-gray-500 text-sm flex-wrap">
            ...
            <div>
              ...
              {/* overflow-wrap + break-words prevents long strings from bleeding into the right column */}
              <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-wrap break-words overflow-hidden">
                ...
                <div className="flex flex-wrap gap-2">
                  {property.amenities.map((a) => {
                    ...
                    return (
                      <span
                        key={a}
                        className="inline-flex items-center gap-1.5 text-sm px-3 py-1.5 bg-white border border-gray-200 rounded-full text-gray-700 shadow-sm"
                      >
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-gray-900 truncate">{property.contact.name}</p>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-full flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 active:bg-green-700 text-white text-sm font-semibold py-2.5 rounded-xl transition-colors"
              >
