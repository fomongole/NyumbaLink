...
export default function ImageUploadManager({ open, onClose, property, inline = false }: Props) {
  ...
  // Fetch fresh property data to immediately reflect successful uploads/deletions
  const { data: freshProperty } = useQuery({
    queryKey: ['properties', property?.id],
    queryFn: () => propertiesApi.getOne(property.id),
    enabled: open && !!property?.id,
  });

  const activeProperty = freshProperty ?? property;
  const images = activeProperty?.images ?? [];
  ...
  const uploadMutation = useMutation({
    mutationFn: (files: File[]) => mediaApi.uploadImages(activeProperty.id, files),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (imageId: string) => mediaApi.deleteImage(activeProperty.id, imageId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['properties'] });
  ...
  const primaryMutation = useMutation({
    mutationFn: (imageId: string) => mediaApi.setPrimary(activeProperty.id, imageId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['properties'] });
    ...
    <div className={inline ? "space-y-4" : ""}>
      ...
      {slotsRemaining === 0 && (
        <p className="text-xs text-center text-gray-400 mt-2">
          Maximum of 4 images reached. Delete one to upload more.
        </p>
      )}
    </div>
  );
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Manage Images — {activeProperty.title}</DialogTitle>
        </DialogHeader>
        {content}
      </DialogContent>
    </Dialog>
